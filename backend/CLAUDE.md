...
- **CORS:** `CORSMiddleware` is mounted with `allow_origins=["*"]` so the frontend can be deployed on a different host (see root `CLAUDE.md` → Deployment). Safe to leave wide open as long as v1 stays auth-free/cookie-free; tighten this if that changes.